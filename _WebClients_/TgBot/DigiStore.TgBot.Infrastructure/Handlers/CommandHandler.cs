...
public class CommandHandler
{
	...
	public async Task HandleStartCommand(ITelegramBotClient botClient, Message message, CancellationToken ct)
	{
		try
		{
			...
			"Start command from Telegram ID: {TelegramId}, Name: {FirstName} {LastName}",
			...
			_logger.LogInformation(
			"User initialized: TelegramId: {TelegramId}, UserId: {UserId}",
			telegramId, user.Id);
		}
		catch (Exception ex)
		{
	...
	CancellationToken ct)
	{
		try
		{
			...
			cancellationToken: ct);
			_logger.LogInformation(
			"Profile sent for user: {UserId}, TelegramId: {TelegramId}",
			userId, telegramId);
		}
		catch (Exception ex)
		{
	...
	CancellationToken ct)
	{
		try
		{
			...
			await _sessionService.UpdateSessionAsync(session, ct);
			_logger.LogInformation(
			"Language selection sent for user: {TelegramId}",
			telegramId);
		}
		catch (Exception ex)
		{
